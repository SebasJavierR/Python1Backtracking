class Coord:
    ...
    def __iter__(self):
        ...
        >>> assert c == 5
        """
        coordenadas=(self.fila,self.columna)
        iterador = iter(coordenadas)
        return iterador
    ...
    * 0 o más celdas "bloqueadas", que representan las paredes del laberinto
    ...
    def __init__(self, filas, columnas):
        ...
        for x in range(columnas):
            for y in range(filas):
                self.posiciones[Coord(y,x)]= self.NO_VISITADA
    ...
    def celda_bloqueada(self, coord):
        ...
        if not self.es_coord_valida(coord):
            raise ValueError("coordenada no valida")
        elif self.posiciones[coord] == self.BLOQUEADA:
            return True
        else: 
            return False
    ...
    def visitar(self, coord):
        ...
        if self.celda_bloqueada(coord):
            self.posiciones[coord]= self.VISITADA
